            ...
            <ul class="nav nav-tabs card-header-tabs">
                ...
                <li class="nav-item">
                    <button class="nav-link {{ $activeTab === 'consultations' ? 'active fw-bold' : '' }}" wire:click="switchTab('consultations')">
                        ...
                        <span class="badge bg-secondary ms-1">{{ $selectedDossier->consultations->count() }}</span>
                    </button>
                </li>
                <li class="nav-item">
                ...
                <li class="nav-item">
                    <button class="nav-link {{ $activeTab === 'rendezvous' ? 'active fw-bold' : '' }}" wire:click="switchTab('rendezvous')">
                        ...
                        <span class="badge bg-secondary ms-1">{{ $selectedDossier->rendezVous->count() }}</span>
                    </button>
                </li>
            </ul>
        </div>
...
@if($activeTab === 'consultations')
    @php
        $consultationsList = $selectedDossier->consultations ?? collect();
    @endphp

    <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="fw-bold text-primary mb-0">
            <i class="bi bi-file-earmark-medical me-1"></i> Consultations du Dossier
        </h6>
        <span class="badge bg-light text-dark border">
            Total : {{ $consultationsList->count() }}
        </span>
    </div>
        ...
        <table class="table table-hover align-middle border">
            <thead class="table-light">
                <tr>
                    ...
                    <th>Motif</th>
                    <th>Diagnostic</th>
                    <th>Constantes / Remarques</th>
                </tr>
            </thead>
            <tbody>
                @forelse($consultationsList as $c)
                    <tr>
                        <td>
                            <div class="fw-bold text-dark">
                                <i class="bi bi-calendar-event me-1 text-primary"></i>
                                @php
                                    $dateRaw = $c->date_consultation ?? $c->created_at;
                                @endphp
                        ...
                        <td>
                            <div class="fw-semibold">
                                Dr. {{ $c->medecin->nom ?? $c->medecin->name ?? 'Non assigné' }} {{ $c->medecin->prenom ?? '' }}
                            </div>
                            @if(!empty($c->medecin->specialite))
                                <small class="text-muted d-block">{{ $c->medecin->specialite }}</small>
                            @endif
                        </td>
                        <td>
                            <span class="text-wrap">{{ $c->motif ?: 'Non précisé' }}</span>
                        </td>
                        <td>
                            @if($c->diagnostic)
                                <span class="fw-semibold text-dark">{{ $c->diagnostic }}</span>
                            @else
                        ...
                        <td>
                            <small class="text-muted">
                                {{ $c->notes ?? $c->observation ?? $c->conclusion ?? '-' }}
                            </small>
                        </td>
                    </tr>
                @empty
